...
export default function NewChat() {
  ...
  return (
    <div className="flex flex-col h-screen pt-10">
        ...
        <div className="flex flex-1 h-full flex-col md:pr-20 md:pl-10 gap-y-5 w-full">
          ...
          {messages.map((message) => (
            ...
            >
              ...
              <Card className={`bg-pink-500 px-6 py-3 text-sm font-medium text-white shadow-sm transition-colors focus:outline-none dark:bg-purple-500 border-none ${message.role === "user" ? 'mr-[-1.8rem]' : 'ml-[-1.8rem]'}`}>
                {message.role === "assistant" ? (
      ...
      <form className="flex items-center gap-4 mb-[5rem] md:px-10" onSubmit={handleSubmit}>
        <Input
