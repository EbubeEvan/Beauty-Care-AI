...
messageSchema.pre('validate', function (next) {
  ...
  const hasAttachments =
    Array.isArray(this.experimental_attachments) && this.experimental_attachments.length > 0;

  if (!hasText && !hasParts && !hasAttachments) {
    const err = new Error('A message must have content, parts, or experimental_attachments');
    return next(err);
  }
  next();
});
...
chatHistorySchema.methods.addMessage = function (this: IChatHistory, message: StoredMessage) {
  this.messages.push(message);
  return this.save();
};

chatHistorySchema.static('findByUserId', function (this: ChatHistoryModel, userId: string) {
  return this.find({ userId }).sort({ createdAt: -1 });
});

chatHistorySchema.static('findByChatId', function (this: ChatHistoryModel, chatId: string) {
  return this.findOne({ chatId });
});

const ChatHistory =
  (models.ChatHistory as ChatHistoryModel) ||
  model<IChatHistory, ChatHistoryModel>('ChatHistory', chatHistorySchema);
